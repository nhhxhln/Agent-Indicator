...
/* ---- 覆盖灯效(OpenRGB 风格预设);两路独立:[0]=matrix [1]=aux ---- */
static volatile led_fx_t s_fx[2] = { LED_FX_AGENT, LED_FX_AGENT };
static rgb_t s_fx_color[2] = { { 0, 140, 255 }, { 0, 140, 255 } };
...
void led_engine_set_fx_target(led_target_t tg, led_fx_t fx,
                              uint8_t r, uint8_t g, uint8_t b, uint8_t speed)
{
    if (fx >= LED_FX_MAX) return;
    for (int s = 0; s < 2; s++) {
        if (tg != LED_TGT_ALL && (int)tg != s + 1) continue; /* 1=matrix(s0) 2=aux(s1) */
        s_fx_color[s] = (rgb_t){ r, g, b };
        if (speed >= 1 && speed <= 100) s_fx_speed[s] = speed;
        s_fx[s] = fx;
    }
}
...
{
    led_engine_set_fx_target(LED_TGT_ALL, fx, r, g, b, speed);
}

led_fx_t led_engine_get_fx(void) { return s_fx[0]; }
led_fx_t led_engine_get_fx_target(led_target_t tg) { return s_fx[tg == LED_TGT_AUX ? 1 : 0]; }

void led_engine_flash(uint8_t r, uint8_t g, uint8_t b, int times)
{
    if (times < 1) times = 1;
    s_flash_color = (rgb_t){ r, g, b };
    s_flash_ticks = times * 2 * FLASH_HALF;
}
...
/* 把一条灯带(buf[0..n))按指定灯效渲染(matrix/aux 各自独立调用) */
static void render_fx_strip(rgb_t *buf, int n, led_fx_t fx, rgb_t color,
                            uint8_t speed, float t)
{
    float spd = speed / 40.0f;
    for (int i = 0; i < n; i++) {
        rgb_t c = { 0, 0, 0 };
        switch (fx) {
        case LED_FX_SOLID:
        ...
        case LED_FX_BREATH: {
            ...
            c = scale(color, (uint8_t)(k * 255));
            break;
        }
        case LED_FX_MARQUEE: {
            int pos = (int)(t * 24 * spd);
            c = (((i - pos) % 12 + 12) % 12) < 4 ? color : scale(color, 18);
            break;
        }
        case LED_FX_RAINBOW:
            c = hue_to_rgb((uint8_t)(i * 256 / n + (int)(t * 64 * spd)));
            break;
        default: /* OFF */
            break;
        }
...
static void led_task(void *arg)
{
    ...
    while (1) {
        ...
        if (s_flash_ticks > 0) {
            /* 通知爆闪:两路全亮指定色与黑交替,盖过常规灯效 */
            rgb_t c = ((s_flash_ticks / FLASH_HALF) & 1) ? s_flash_color
                                                         : (rgb_t){ 0, 0, 0 };
            for (int i = 0; i < mleds; i++) s_mbuf[i] = c;
            for (int i = 0; i < AUX_LEDS; i++) s_abuf[i] = c;
            s_flash_ticks--;
        } else {
            /* matrix(路 0) */
            if (s_fx[0] == LED_FX_AGENT) render_matrix(t);
            else render_fx_strip(s_mbuf, mleds, s_fx[0], s_fx_color[0], s_fx_speed[0], t);
            /* aux(路 1):ring+usage+vu */
            if (s_fx[1] == LED_FX_AGENT) { render_ring(t); render_usage(t); render_vu(); }
            else render_fx_strip(s_abuf, AUX_LEDS, s_fx[1], s_fx_color[1], s_fx_speed[1], t);
        }
